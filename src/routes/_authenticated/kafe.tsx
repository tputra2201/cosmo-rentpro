...
export const Route = createFileRoute("/_authenticated/kafe")({
  head: () => ({
    meta: [
      { title: "Kafe & Nomor Meja — Billing Rental PS" },
      {
        name: "description",
        content:
          "Layani pelanggan kafe tanpa rental PlayStation: pilih nomor meja, catat pesanan per kategori menu, lalu selesaikan pembayaran.",
      },
      { property: "og:title", content: "Kafe & Nomor Meja" },
      {
        property: "og:description",
        content:
          "Pesanan kafe per nomor meja dengan kategori menu yang bisa diatur sendiri.",
      },
...
function tableTotal(table: CafeTable) {
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-3xl font-bold sm:text-4xl">Kafe &amp; Nomor Meja</h1>
          <p className="mt-1 text-muted-foreground">
            Untuk pelanggan yang hanya makan atau ngopi, tanpa sewa PlayStation.
          </p>
        </div>
        <div className="text-right">
          <p className="text-xs text-muted-foreground">
            {openTables.length} meja terisi
          </p>
      ...
      <section className="surface-panel p-6">
        ...
        <ul className="mt-4 space-y-2">
          {cafeTables.map((t) => (
            <li
              key={t.id}
              className="grid gap-2 rounded-lg bg-secondary/60 p-3 sm:grid-cols-[1fr_1fr_120px]"
            >
              <Input
                value={t.name}
